cat > /root/001/my_trading_bot/main.py << 'EOF'
import json
import time
import threading
import requests
...
coin_losses = {}
ema_state = {}
trade_cache = defaultdict(list)   # 存储 aggTrade 成交数据，key: symbol, value: list of trades
    ...
    if os.path.exists(SYMBOLS_CACHE_FILE):
        ...
        if time.time() - mtime < CACHE_TTL:
            try:
                ...
                pass
    # 缓存失效，请求 REST API（只在启动时或缓存过期时发生）
    try:
...
if __name__ == "__main__":
    print("🚀 启动优化版策略（WebSocket成交数据）")
    notify("🚀 启动优化版策略（WebSocket成交数据）")
