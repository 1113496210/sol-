...
MAX_1H_CHANGE = 0.10   # 追高否决阈值（8% → 10%）
...
def analyze_symbol(symbol):
    try:
        ...
        if bullish >= 3:
            ...
            bear += 1
        # 突破确认：价格高于过去20根K线最高收盘价
        recent_high_close = max(closes[-20:-1]) if len(closes) >= 21 else max(closes[:-1])
        if closes[-1] > recent_high_close:
            bull += 1
        # 资金费率
        ...
        if bull >= MIN_FACTORS_TO_ALERT:
            if price_change_1h > MAX_1H_CHANGE:
                logger.info(f"⛔ {symbol} 否决: 1h涨幅 {price_change_1h*100:.2f}% 超 10%")
                return None
            return {
                "symbol": symbol,
                "direction": "LONG",
        ...
        if bear >= MIN_FACTORS_TO_ALERT:
            if price_change_1h < -MAX_1H_CHANGE:
                logger.info(f"⛔ {symbol} 否决: 1h跌幅 {price_change_1h*100:.2f}% 超 10%")
                return None
            return {
                "symbol": symbol,
                "direction": "SHORT",
...
def main_loop():
    ...
    logger.info(f"🚀 融合监控系统启动（四通道优化版 · 800万门槛）")
    notify_all(
        f"✅ 融合监控系统启动（四通道优化版）\n"
        f"🚀 通道1：确认信号（持续性确认）\n"
