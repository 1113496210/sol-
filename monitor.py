# -*- coding: utf-8 -*-
# =====================================================
# Binance USDT 永续合约监控系统 v3
# 改动：补漏阈值调高 + B 系统 24h 双重过滤 + 全局同币 60 秒静默
# =====================================================
...
B_MIN_TOTAL_PCT = 2.0
B_PROGRESS_PCT = 4.0
B_PROGRESS_INTERVAL = 300
B_DRAWDOWN_FAIL = 0.07
B_MAX_DAILY_ROUND = 1
B_MAX_PUSH = 3
B_MIN_24H_PCT = 3.0      # ✨ 24h 涨幅至少 3%
B_MAX_24H_PCT = 100.0    # ✨ 24h 涨幅小于 100%（屏蔽数据异常）

# ================= 系统 C =================
C_POS_MIN_RATIO = 0.33
C_HIGH_GAP_PCT = 8.0
C_TRIG_A_VOL_MULT = 2.0
C_TRIG_B_BULL_BARS = 4
C_TRIG_B_VOL_MULT = 1.5
C_PROGRESS_PCT = 4.0
...
# ================= ✨ 全局同币静默 =================
GLOBAL_SILENCE_SEC = 60
state_last_alert_ts = {}
state_silence_lock = threading.Lock()

def is_silenced(symbol):
    with state_silence_lock:
        last = state_last_alert_ts.get(symbol, 0)
        return time.time() - last < GLOBAL_SILENCE_SEC

def mark_alerted(symbol):
    with state_silence_lock:
        state_last_alert_ts[symbol] = time.time()
...
def _a_flush():
    ...
    if not items:
        return
    # ✨ 过滤静默期内的币
    items = [it for it in items if not is_silenced(it["sym"])]
    for it in items:
        mark_alerted(it["sym"])
    if not items:
        return
...
# ================= 状态机 =================
def new_daily():
    return {"first_time": None, "first_price": None, "first_src": None,
            "second_time": None, "second_price": None,
            "third_time": None, "third_price": None,
            "high": 0.0, "low": 1e18, "progress_count": 0}
...
def new_c():
    return {"active": False, "last_high": None, "base_price": None,
            "push_count": 0, "daily_round": 0, "start_pct": 0.0,
            "last_progress_price": None, "last_progress_ts": 0,
            "day": None, "trigger_type": None}
    ...
    try:
        ...
        if not sb["active"]:
            ...
            if hh and start_pct >= B_MIN_TOTAL_PCT:
                # ✨ 24h 双重过滤
                try:
                    ch24_raw = float((get_24h_one(sym) or {}).get("priceChangePercent", "0"))
                except:
                    ch24_raw = 0.0
                if ch24_raw < B_MIN_24H_PCT or ch24_raw >= B_MAX_24H_PCT:
                    return
                # ✨ 静默检查
                if is_silenced(sym):
                    return
            ...
            if sb["last_progress_price"]:
                ...
                if (prog_pct >= B_PROGRESS_PCT
                        and time.time() - sb["last_progress_ts"] >= B_PROGRESS_INTERVAL
                        and sb["push_count"] < B_MAX_PUSH):
                    # ✨ 静默检查
                    if is_silenced(sym):
                        return
                    mark_alerted(sym)
...
# ================= 系统 C =================
c_kline_cache_15m = defaultdict(lambda: deque(maxlen=120))
c_kline_cache_3m = defaultdict(lambda: deque(maxlen=60))
...
def c_evaluate(symbol, price):
    ...
    if not sc["active"]:
        ...
        if trig_a or trig_b:
            ...
            sc["trigger_type"] = ("启动型(EMA上穿+爆量)" if trig_a
                                  else "推进型(多头持续+放量)")
        ...
        if sc["last_progress_price"]:
            ...
            if (prog_pct >= C_PROGRESS_PCT
                    and time.time() - sc["last_progress_ts"] >= C_PROGRESS_INTERVAL
                    and sc["push_count"] < C_MAX_PUSH):
                # ✨ 静默检查
                if is_silenced(symbol):
                    return
                mark_alerted(symbol)
...
def on_ws_kline(symbol, interval, kd):
    ...
    if interval == "15m":
        ...
        c_kline_cache_3m[symbol].append({"h": kd["h"], "l": kd["l"],
                                         "c": kd["c"], "v": kd["v"]})
        c_evaluate(symbol, kd["c"])
...
def boot_rescan(symbols):
    ...
    try:
        all_tickers = {t["symbol"]: float(t.get("priceChangePercent", 0))
                       for t in get_24h_tickers()}
    except Exception as e:
        print(f"[BOOT] 拉 24h ticker 失败: {e}")
        all_tickers = {}
    cnt = 0
    for sym in symbols:
        try:
            ...
            base = closes[-17]
            price = closes[-1]
            pct_4h = (price - base) / base * 100
            pct_24h = all_tickers.get(sym, 0)
            if pct_4h >= BOOT_RISE_PCT and pct_24h >= 5.0:
                # ✨ 静默检查
                if is_silenced(sym):
                    continue
                mark_alerted(sym)
...
def reset_daily_states():
    global state_a_played, state_range_played, state_last_alert_ts
    with state_lock:
        state_daily.clear()
    state_a_played = set()
    state_range_played = defaultdict(set)
...
def generate_report():
    try:
        with state_lock:
            data = {sym: dict(d) for sym, d in state_daily.items()
                    if d["first_price"] is not None}
        if not data:
        ...
        send_email_attachment(f"监控日报 {today_cn()}",
                              f"附件为 {today_cn()} 信号日报,共 {len(df)} 个币种触发。",
                              fname)
